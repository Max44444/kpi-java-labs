...
public class Validator {
    public static void notEmpty(String candidate) {
    ...
    public static void checkPrice(BigDecimal candidate) {
    ...
    public static void checkUpperBound(long candidate, long bound) {
        if (candidate > bound) {
            throw new IncorrectNumberException("The number should not be grates then " + bound);
        }
    }

    public static void checkUpperBound(Calendar candidate, Calendar bound) {
        if (candidate.after(bound)) {
            throw new IncorrectNumberException("The date should not be after "
                + (new SimpleDateFormat("dd.MM.yyyy").format(bound.getTime())));
        }
    }

    public static void checkLowerBound(long candidate, long bound) {
        if (candidate < bound) {
            throw new IncorrectNumberException("The number should not be less then " + bound);
        }
    }

    public static void checkLowerBound(Calendar candidate, Calendar bound) {
        if (candidate.before(bound)) {
            throw new IncorrectNumberException("The date should not be before "
                + (new SimpleDateFormat("dd.MM.yyyy").format(bound.getTime())));
        }
    }

    public static void checkTour(Tour candidate) {
        try {
